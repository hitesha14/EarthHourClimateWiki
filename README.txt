...
1) Climate Wiki is based on Media Wiki. It basically runs on Apache, Php and MySql
2)Please install MediaWiki from Bitnami instructions are given below. 
3) Once after the installation is done.Please download the climatewiki files from the repository(Climate Wiki) of GitHub.
4) Replace them with the existing files in the folder
4) Execute the mysql exe files and run the Apache Server.
5) Paste the following URL http://localhost/mediawiki/index.php/Main_Page  in the browser to access Climate Wiki
            


Bitnami MediaWiki Stack 1.23.3-0
                  ==============================
...
bitnami-mediawiki-1.23.3-0-linux-installer.run on Linux or
bitnami-mediawiki-1.23.3-0-linux-x64-installer.run on Linux 64 bit or
bitnami-mediawiki-1.23.3-0-windows-installer.exe on Windows or
bitnami-mediawiki-1.23.3-0-osx-x86-installer.dmg on OS X x86.

On Linux, you will need to give it executable permissions:

chmod 755 bitnami-mediawiki-1.23.3-0-linux.run
...
Stack in /home/user/mediawiki-1.23.3-0 on Linux or C:\Program Files\Bitnami MediaWiki Stack
on Windows or /Applications/mediawiki-1.23.3-0 and that you use port 
...
"Cannot open bitnami-mediawiki-1.23.3-0-linux.run: No application suitable for
...
/home/user/mediawiki-1.23.3-0/apache2/logs/error.log on Linux or
C:\Program Files\Bitnami MediaWiki Stack\apache2\logs\error.log on Windows or
/Applications/mediawiki-1.23.3-0/apache2/logs/error.log on OS X.
